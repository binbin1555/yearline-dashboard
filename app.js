...
const MA_N = 250, TAKE_PROFIT = 0.80;
const VOL_N = 20, VOL_K = 1.3;    // v3.0：当日成交额 >= 前20日均额 × 1.3
...
// v3.0 进场条件②：当日成交额 >= 前20日均额 × 1.3
// 成交额缺失时返回 false —— 拿不到量能数据就不进场（保守默认）
const volOk = (d, ind, i) =>
  ind.av[i] != null && d.cyb_amt[i] != null && d.cyb_amt[i] >= VOL_K * ind.av[i];
...
function run(d, ind, startDate, capital) {
  ...
  for (let i = s; i < n; i++) {
    ...
    if (!pend) {
      ...
      if (state === 'HL') {
        // v3.0：站上均线 + 放量。均线斜率不再参与判定。
        if (d.cyb_sig[i] > ma[i] && volOk(d, ind, i)) pend = ['CYB', nxt, i];
...
function nextTriggers(d, ind, res) {
  ...
  if (res.state === 'CYB') {
    ...
    // v3.0 进场需同时满足「站上均线」和「放量」，取更难的那个作为距离
    const amt = d.cyb_amt[i];
    const ratio = (av[i] && amt != null) ? amt / av[i] : null;
    const priceProg = clamp(cyb / ma[i], 0, 1);
    const volProg = ratio ? clamp(ratio / VOL_K, 0, 1) : 0;
    const priceOk = cyb > ma[i];
    const volBinding = volProg <= priceProg;
    const upPct = (ma[i] / cyb - 1) * 100;
    let headline, short;
    if (ratio == null) {
      headline = '成交额数据缺失，无法判定';
      short = '数据缺失';
    } else if (volBinding) {
      headline = `成交额 <b>${ratio.toFixed(2)}×</b> ／ 需 ${VOL_K.toFixed(2)}×`;
      short = `量能 ${ratio.toFixed(2)}×／${VOL_K.toFixed(2)}×`;
    } else {
      headline = `再涨 <b>${upPct.toFixed(2)}%</b> 站上 MA250`;
      short = `再涨 ${upPct.toFixed(2)}%`;
    }
    const cond = priceOk
      ? `已站上 MA250 ${ma[i].toFixed(2)}，只差放量至 ${VOL_K.toFixed(2)}×`
      : `需站上 MA250 ${ma[i].toFixed(2)}，且成交额达 ${VOL_K.toFixed(2)}×`;
    out.push({ label: '创业板进场', action: `全仓买入${NAME.cyb}`,
      cond, headline, short,
      progress: Math.min(priceProg, volProg),
      marks: [`当日 ${amt != null ? yi(amt) : '—'}`, '',
              av[i] ? `需 ${yi(VOL_K * av[i])}` : ''] });
